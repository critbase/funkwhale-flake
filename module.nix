...
let
  pythonEnv = (pkgs.python38.override {
  ...
  databasePassword = if (cfg.database.passwordFile != null) then
  ...
  databaseUrl =
    if (cfg.database.createLocally && cfg.database.socket != null) then
      ...
      "postgresql://${cfg.database.user}:${databasePassword}@${cfg.database.host}:${
...
in {
    ...
    services.funkwhale = {
      ...
      database = {
        ...
        socket = mkOption {
          ...
          description =
            "Path to the unix socket file to use for authentication for local connections.";
        };
      ...
      api = {
        ...
        djangoSecretKey = mkOption {
          ...
          '';
          example =
            "6VhAWVKlqu/dJSdz6TVgEJn/cbbAidwsFvg9ddOwuPRssEs0OtzAhJxLcLVC";
        };
      };
      ...
      ravenDsn = mkOption {
        type = types.str;
        default =
          "https://44332e9fdd3d42879c7d35bf8562c6a4:0062dc16a22b41679cd5765e5342f716@sentry.eliotberriot.com/5";
        description = ''
          Sentry/Raven DSN.
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        ...
        message =
          "one of services.funkwhale.database.socket, services.funkwhale.database.passwordFile, or services.funkwhale.database.password must be set";
      }
      {
        assertion = cfg.database.createLocally -> cfg.database.user == cfg.user;
        message =
          "services.funkwhale.database.user must be set to ${cfg.user} if services.funkwhale.database.createLocally is set true";
      }
      {
        assertion = cfg.database.createLocally -> cfg.database.socket != null;
        message =
          "services.funkwhale.database.socket must be set if services.funkwhale.database.createLocally is set to true";
      }
      {
        assertion = cfg.database.createLocally -> cfg.database.host
          == "localhost";
        message =
          "services.funkwhale.database.host must be set to localhost if services.funkwhale.database.createLocally is set to true";
      }
    ];
    ...
    users.users.funkwhale =
      mkIf (cfg.user == "funkwhale") { group = cfg.group; };
    ...
    in {
      ...
      funkwhale-beat = {
        ...
        serviceConfig = serviceConfig // {
          ...
          ExecStart = ''
            ${pythonEnv}/bin/celery -A funkwhale_api.taskapp beat \
  ...
  meta = { maintainers = with lib.maintainers; [ mmai ]; };
}
